...
void vaastaanotin() {
  ...
  int nackCounter = 3; //Nack counter monta uudelleenlähteystä
  int waitForTx=0;
  while(true) {
    ...
    if(!(waitForTx%10))
    {
      ...
      std::cout << "Input new message: " << std::endl;
          	...
          	std::cout << "NACK tried 3 times Ending transmission!" << std::endl; //
          	s_finished = true;
          }
        }
    ...
    auto tmp = std::move(rxQueue.readItem()); // reads the received data from PHY
      ...
      if((to_byte(header.first) == 0x00) && (to_byte(header.second) == 0x00)) {  // Onhan data paketti
        ...
        if (packet_correctness.back()) std::cout << "You received a message: ";
...
int UHD_SAFE_MAIN(int argc, char *argv[]) 
{
  ...
  while(true){
    ...
    int timeOutCount = 10; // laskee monta timeouttia tulee ettei jää pyörimään loputtomasti
		...
		if(tmp != nullptr)                        // received a data packet
    {
      ...
      */
      ...
      if((to_byte(header.first) == 0x06) && (to_byte(header.second) == 0x06)) {
        ...
        vstn.join();
        //std::cout << "SULKEMINEN ONNISTUI(?)" << std::endl;
        std::cout << std::endl;
        transmit_on = true;
        //std::cout << "SULKEMINEN ONNISTUI(?)" << std::endl;
      }
